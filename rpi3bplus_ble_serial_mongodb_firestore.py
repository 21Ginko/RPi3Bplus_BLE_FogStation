...
mongo_client = MongoClient("") #Change with your own
...
def is_internet_available():
    try:
        ...
        # Connection error, no internet available
        return False
                    ...
                    if data["data"].startswith("S"):
                        ...
                        if len(parts) == 2:
                            ...
                            if sensor.exists:
                                ...
                                last_sensor_reading_ref = db.collection("users").document(users[0]["UID"]).collection("last_sensor_readings").document(str(sensor_id))
                                last_sensor_reading = last_sensor_reading_ref.get()
                                
                                # Check if the collection exists, and create it if not
                                if not last_sensor_reading.exists:
                                # Create a new sensor reading
                                    ...
                                    # Add the sensor reading to the user's "last_sensor_readings" collection
                                    last_sensor_reading_ref.set(new_sensor_reading)
                                else: 
                                    ...
                                    new_sensor_reading = {
                                        "created_at":firestore.SERVER_TIMESTAMP,
                                        "reading": reading,
                                        "previous_reading": last_sensor_reading.to_dict().get("reading"),
                                        "sensor_info": sensor.to_dict(),
                                        "timestamp": data["created_at"]
                                    }
